...
namespace NetProxy
{
    public class VPNAPIioHelper
    {
        private static string VPNAPIioApiKey { get; set; } // Replace with your VPNAPI.io API key
        private const string DatabaseFile = "VPNAPIioCache.db";

        public VPNAPIioHelper()
        {
            InitializeDatabase();
            VPNAPIioApiKey = Program.dataSettings["Proxy-Checking"]["api_key"];
        }
        ...
        public async Task<(bool IsVpn, string Country)> IsVpnIp(IPAddress ip)
        {
            ...
            // Call the VPNAPI.io API if not in cache or outdated
            using (HttpClient client = new HttpClient())
            {
                try
                {
                    string url = $"https://vpnapi.io/api/{ipString}?key={VPNAPIioApiKey}";
                    ...
                    bool isVpn = jsonResponse["security"]?["vpn"]?.ToObject<bool>() ?? false;
                    string country = jsonResponse["location"]?["country_code"]?.ToString() ?? "Unknown";
