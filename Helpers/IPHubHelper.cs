...
namespace NetProxy
{
    public class IPHubHelper
    {
        private static string IPHubApiKey { get; set; } // Replace with your IPHub API key
        private const string DatabaseFile = "IPHubCache.db";

        public IPHubHelper()
        {
            InitializeDatabase();
            IPHubApiKey = Program.dataSettings["Proxy-Checking"]["api_key"];
        }
        ...
        public async Task<(bool IsVpn, string Country)> IsVpnIp(IPAddress ip)
        {
            ...
            // Call the IPHub API if not in cache or outdated
            using (HttpClient client = new HttpClient())
            {
                try
                {
                    string url = $"https://v2.api.iphub.info/ip/{ipString}";
                    client.DefaultRequestHeaders.Add("X-Key", IPHubApiKey);
                    ...
                    bool isVpn = jsonResponse["block"]?.ToObject<int>() == 1;
                    string country = jsonResponse["countryCode"]?.ToString() ?? "Unknown";
