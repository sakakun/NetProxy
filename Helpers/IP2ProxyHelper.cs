...
namespace NetProxy
{
    public class IP2ProxyHelper
    {
        private static string IP2ProxyApiKey { get; set; } // Replace with your IP2Proxy API key
        private const string DatabaseFile = "IP2ProxyCache.db";

        public IP2ProxyHelper()
        {
            InitializeDatabase();
            IP2ProxyApiKey = Program.dataSettings["Proxy-Checking"]["api_key"];
        }
        ...
        public async Task<(bool IsVpn, string Country)> IsVpnIp(IPAddress ip)
        {
            ...
            // Call the IP2Proxy API if not in cache or outdated
            using (HttpClient client = new HttpClient())
            {
                try
                {
                    string url = $"https://api.ip2proxy.com/?key={IP2ProxyApiKey}&ip={ipString}&package=PX1&format=json";
                    ...
                    bool isVpn = jsonResponse["is_proxy"]?.ToString() == "1";
