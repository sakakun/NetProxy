...
namespace NetProxy
{
    public class IPQualityScoreHelper
    {
        private static string IPQualityScoreApiKey { get; set; } // Replace with your IPQualityScore API key
        private const string DatabaseFile = "IPQualityScoreCache.db";

        public IPQualityScoreHelper()
        {
            InitializeDatabase();
            IPQualityScoreApiKey = Program.dataSettings["Proxy-Checking"]["api_key"];
        }
        ...
        public async Task<(bool IsVpn, string Country)> IsVpnIp(IPAddress ip)
        {
            ...
            // Call the IPQualityScore API if not in cache or outdated
            using (HttpClient client = new HttpClient())
            {
                try
                {
                    string url = $"https://ipqualityscore.com/api/json/ip/{IPQualityScoreApiKey}/{ipString}";
                    ...
                    bool isVpn = jsonResponse["vpn"]?.ToObject<bool>() ?? false;
